from config import players_name, rapid_api, project_id, dataset_id
from google.cloud import bigquery
import pandas as pd
import requests
import json

def call_players_api():
...
class Players:
	def load(self):
		...
		table_id = f"{project_id}.{dataset_id}.{players_name}"
	...
	def drop(self):
		client = bigquery.Client(project=project_id)
		query = f"""
            DROP TABLE {project_id}.{dataset_id}.{players_name}
		"""
